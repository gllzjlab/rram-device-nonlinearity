...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0,
                 scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True,
    ...
    def saveckpt(self, path='model'):
        ...
        dic = dict(
            ...
            is_extend_grid = model.is_extend_grid,
            coef_negative = model.coef_negative         
        )
    ...
    @staticmethod
    def loadckpt(path='model', activation_config=None):
        '''
        ...
        model_load = MultKAN(config['width'], 
                     grid=config['grid'], 
                     k=config['k'], 
                     ...
                     is_extend_grid = config['is_extend_grid'],
                     coef_negative = config['coef_negative'],
    ...
    def plot(self, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0, save_name=None):
        '''
        plot KAN
        ...
        plt.savefig(os.path.join(folder, save_name), bbox_inches="tight", dpi=400)
        plt.close()  
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1.,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
              ...
              save_best=False, save_best_key=None, save_best_path=None):
        '''
        training
        ...
        if save_best:
            best_val = -1e9
            # choose default key: prefer provided save_best_key, else try metrics last, else 'test_loss'
            if save_best_key is None:
                if metrics is not None and len(metrics) > 0:
                    default_key = metrics[-1].__name__
                else:
                    default_key = 'test_loss'
                save_best_key = default_key
            if save_best_path is None:
                save_best_path = self.ckpt_path + '/best_model'
        ...
        for _ in pbar:
            ...
            if metrics != None:
                ...
                # check and save best model if requested
            if save_best:
                if save_best_key in results:
                    curr_val = results[save_best_key][-1]
                else:
                    curr_val = None
                try:
                    # higher is better for accuracy-like metrics
                    if curr_val is not None and curr_val > best_val:
                        best_val = curr_val
                        # try to use library checkpoint saver first
                        try:
                            self.saveckpt(path=save_best_path)
                        except Exception:
                            torch.save(self.state_dict(), f'{save_best_path}_state_dict.pth')
                except Exception:
                    pass
