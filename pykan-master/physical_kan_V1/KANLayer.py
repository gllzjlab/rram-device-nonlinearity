...
import numpy as np
from .physical_action_multi import BatchedUnifiedSmoothActivation, BatchedUnifiedSmoothActivation_1, UnifiedSmoothActivationWithCorrection, UnifiedSmoothActivationWithCorrection_1
...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, scale_base_mu=0.0, scale_base_sigma=1.0,
                 scale_sp=1.0, base_fun=torch.nn.SiLU(), sp_trainable=True, sb_trainable=True,
                 save_plot_data = True, device='cpu', sparse_init=False,
                 coef_negative =True, activation_config = None):
        ''''
        ...
        self.bn = nn.BatchNorm1d(self.in_dim, affine=True)

        if activation_config:
            if coef_negative:

                self.activation = BatchedUnifiedSmoothActivation_1(
                    num_instances = self.in_dim * self.out_dim,
                    **activation_config
                )
            else:
                self.activation = BatchedUnifiedSmoothActivation(
                    num_instances=self.in_dim * self.out_dim,
                    **activation_config
                )
    ...
    def forward(self, x):
        ...
        act_input = act_input[:,None,:].clone().expand(batch, self.out_dim, self.in_dim).reshape((batch, -1))

        y = self.activation(act_input).reshape(batch, self.in_dim, self.out_dim)
    ...
    def get_subset(self, in_id, out_id):
        ...
        (2, 3)
        '''
        spb = KANLayer(len(in_id), len(out_id), base_fun=self.base_fun)
