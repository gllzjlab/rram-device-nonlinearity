...
class MultKAN(nn.Module):
    ...
    -----------
        ...
        device : str
    '''
    def __init__(self, width=None, mult_arity = 2,
                 scale_base_mu=0.0, scale_base_sigma=1.0,
                 base_fun='silu', symbolic_enabled=True, affine_trainable=False,
                 sp_trainable=True, sb_trainable=True, seed=1,
                 save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu',
                 coef_negative=False, activation_config = None):
        '''
        ...
        for l in range(self.depth):
            ...
            sp_batch = KANLayer(in_dim=width_in[l], out_dim=width_out[l+1],
                                ...
                                coef_negative =coef_negative, activation_config = activation_config)
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        for l in range(self.depth):
            ...
            self.act_fun[l].coef.data = another_model.act_fun[l].scale_base.data
    ...
    def reg(self, reg_metric, lamb_l1, lamb_entropy, lamb_coef, lamb_coefdiff):
        ...
        for i in range(len(self.act_fun)):
            coeff_l1 = torch.sum(torch.mean(torch.abs(self.act_fun[i].activation.coef), dim=1))
            coeff_diff_l1 = torch.sum(torch.mean(torch.abs(torch.diff(self.act_fun[i].activation.coef)), dim=1))
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, loss_fn=None, lr=1., batch=-1,
