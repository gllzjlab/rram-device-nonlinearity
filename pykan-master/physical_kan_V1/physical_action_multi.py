...
# ========== 批量激活函数核心类 ==========
class BatchedUnifiedSmoothActivation(nn.Module):
    def __init__(self,
                 num_instances,
                 args,
        ...
        """
        批量版 UnifiedSmoothActivationWithCorrection
        支持 num_instances 个独立激活函数，共享物理结构，独立可学习参数。

        Args:
            num_instances: 激活函数实例总数（如 output_dim * input_dim）
            其余参数同原始 UnifiedSmoothActivationWithCorrection
        """
        super().__init__()
        self.args = args
        ...
        if args.physical_function == 'set':
        ...
        elif args.physical_function == 'reset':
        ...
        elif args.physical_function == 'all':
            ...
            raise ValueError(f"Unknown physical_function: {args.physical_function}")

        # ===== 可学习参数：每个 instance 一份 =====
        init_weight = 1.0 / self.K
        self.coef = nn.Parameter(torch.full((num_instances, self.K), init_weight))

        if getattr(args, 'activation_gains', False):
            self.gains = nn.Parameter(torch.ones(num_instances, self.K))

        #
        self.clamp_min = -3.5
        self.clamp_max = 3.5

        if getattr(args, 'clamp_method', None) == 'scale_bias':
            self.scale = nn.Parameter(torch.ones(num_instances))
            self.bias = nn.Parameter(torch.zeros(num_instances))

        # 注册 centers（如果启用）
        if getattr(args, 'softmax_center', False):
            centers = []
    ...
    def _add_segments(self, breakpoints, model_funcs, params, value_domain, corrections, bound_values, prefix):
        """添加一组 segment（set 或 reset），与原始逻辑一致"""
    ...
    def forward(self, x):
        """
        x: [B, num_instances]
        returns: [B, num_instances]
        """
        B, N = x.shape
        assert N == self.num_instances, f"Expected {self.num_instances} instances, got {N}"

        # Transpose to [N, B] for per-instance processing
        x_t = x.t()  # [N, B]

        # --- Clamp / transform per instance ---
        if self.args.clamp_method == 'scale_bias':
            x_t = self.scale.unsqueeze(1) * x_t + self.bias.unsqueeze(1)
            clamp_min = self.clamp_min.unsqueeze(1)  # [N, 1]
            clamp_max = self.clamp_max.unsqueeze(1)
        elif self.args.clamp_method == 'hard':
            # clamp_min = self.clamp_min.unsqueeze(1)
            # clamp_max = self.clamp_max.unsqueeze(1)
            clamp_min = self.clamp_min
            clamp_max = self.clamp_max
            x_t = torch.clamp(x_t, clamp_min, clamp_max)
        elif self.args.clamp_method == 'software':
            x_t = self.map_function(x_t)
            clamp_min = torch.tensor(0.0, device=x.device, dtype=x.dtype)
            clamp_max = torch.tensor(1.0, device=x.device, dtype=x.dtype)
        else:
            clamp_min = self.clamp_min.unsqueeze(1)
            clamp_max = self.clamp_max.unsqueeze(1)

        # --- Compute weights per instance ---
        if self.args.softmax_center:
            # x_t: [N, B], centers: [K]
            x_exp = x_t.unsqueeze(1)                     # [N, 1, B]
            centers_exp = self.centers.view(1, self.K, 1)  # [1, K, 1]
            distances = (x_exp - centers_exp) / self.sigma
            logits = -distances ** 2
            weights_exp = torch.softmax(logits, dim=1)   # [N, K, B]
        else:
            weights = torch.softmax(self.coef, dim=1)    # [N, K]
            weights_exp = weights.unsqueeze(2)           # [N, K, 1] → broadcast over B

        # --- Compute outputs for each segment ---
        outputs_list = []
        for k, seg in enumerate(self.segments):
            # Determine seg_min / seg_max with your logic
            if seg['prefix'] == 'set':
            ...
            # Map x from [clamp_min, clamp_max] to [seg_min, seg_max]
            denom = clamp_max - clamp_min + 1e-12
            x_mapped = (x_t - clamp_min) / denom * (seg_max - seg_min) + seg_min  # [N, B]

            # Apply base function (vectorized!)
            func = seg['func']
            param_tensors = [
                torch.tensor(p, dtype=x.dtype, device=x.device)
                for p in seg['params']
            ]
            base_out = func(x_mapped, *param_tensors)  # [N, B]

            # Apply correction if enabled
            if self.correction_enabled[k]:
                corr = seg['corrections']
                ...
                base_out = base_out + offset_y + offset_dy * delta_x

            # Normalize by bound_max
            if getattr(self.args, 'activation_gains', False):
                gain_k = self.gains[:, k].unsqueeze(1)  # [N, 1]
                final_out = (base_out / bound_max) * gain_k
            else:
                final_out = base_out / bound_max

            outputs_list.append(final_out)

        outputs_stack = torch.stack(outputs_list, dim=1)  # [N, K, B]
        fused_output = torch.sum(weights_exp * outputs_stack, dim=1)  # [N, B]
        return fused_output.t()  # [B, N]
...
class UnifiedSmoothActivationWithCorrection(nn.Module):
    def __init__(self,
        ...
        init_weight = 1.0 / self.K
        self.coef = nn.Parameter(torch.full((self.K,), init_weight))
    ...
    def forward(self, x, clamp_min=None, clamp_max=None):
        ...
        ###########softmax 正权重##################
            ...
            weights = torch.softmax(self.coef, dim=0)  # [K]
...
class BatchedUnifiedSmoothActivation_1(nn.Module):
    def __init__(self,
                 num_instances,
                 args,
                 breakpoints_set, model_funcs_set, params_set, value_domain_set, corrections_set, bound_values_set,
        ...
        init_coeffs = torch.randn(num_instances, self.K)

        # 对每一行做“和为1”的投影：x_i = x_i - (sum(x_i) - 1) / K
        row_sums = init_coeffs.sum(dim=1, keepdim=True)  # shape: [num_instances, 1]
        init_coeffs = init_coeffs - (row_sums - 1) / self.K

        # 验证：print(init_coeffs.sum(dim=1)) 应该全是 1.0
